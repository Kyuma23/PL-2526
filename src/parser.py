...
def p_program_file_single(p):
    ...
    """
    p[0] = {'node': 'program_file', 'units': [p[1]]}
...
def p_declaration(p):
    ...
    """
    p[0] = {'node': 'declaration', 'type': p[1], 'ids': p[2]}
...
def p_id_decl(p):
    ...
    """
    if len(p) == 2:
        p[0] = {'node': 'id_decl', 'id': p[1]}
    else:
        p[0] = {'node': 'array_decl', 'id': p[1], 'size': p[3]}
...
def p_labeled_stmt_with_label(p):
    ...
    """
    p[0] = {'node': 'labeled_stmt', 'label': p[1], 'statement': p[2]}
...
def p_assign_stmt_array(p):
    ...
    p[0] = {
        'node': 'assign_stmt',
        'target': {'node': 'array_access', 'name': p[1], 'index': p[3]},
        'value': p[6],
    }
...
def p_if_stmt_no_else(p):
    ...
    """
    p[0] = {'node': 'if_stmt', 'condition': p[3], 'then': p[6], 'else': []}
...
def p_if_stmt_with_else(p):
    ...
    """
    p[0] = {'node': 'if_stmt', 'condition': p[3], 'then': p[6], 'else': p[8]}
...
def p_if_stmt_logical(p):
    ...
    """
    p[0] = {'node': 'if_stmt', 'condition': p[3], 'then': [p[5]], 'else': []}
...
def p_print_stmt(p):
    ...
    """
    p[0] = {'node': 'print_stmt', 'values': p[4]}
...
def p_call_stmt_no_args(p):
    ...
    call_stmt : CALL ID
    """
    p[0] = {'node': 'call_stmt', 'name': p[2], 'args': []}
...
def p_call_stmt_with_args(p):
    ...
    """
    p[0] = {'node': 'call_stmt', 'name': p[2], 'args': p[4]}
...
def p_expr_or(p):
    ...
    expr : expr OR expr
    """
    p[0] = {'node': 'binop', 'op': 'OR', 'left': p[1], 'right': p[3]}
...
def p_expr_and(p):
    ...
    expr : expr AND expr
    """
    p[0] = {'node': 'binop', 'op': 'AND', 'left': p[1], 'right': p[3]}
...
def p_expr_not(p):
    ...
    expr : NOT expr
    """
    p[0] = {'node': 'unop', 'op': 'NOT', 'value': p[2]}
...
def p_expr_eq(p):
    ...
    expr : expr EQ expr
    """
    p[0] = {'node': 'binop', 'op': 'EQ', 'left': p[1], 'right': p[3]}
...
def p_expr_ne(p):
    ...
    expr : expr NE expr
    """
    p[0] = {'node': 'binop', 'op': 'NE', 'left': p[1], 'right': p[3]}
...
def p_expr_lt(p):
    ...
    expr : expr LT expr
    """
    p[0] = {'node': 'binop', 'op': 'LT', 'left': p[1], 'right': p[3]}
...
def p_expr_le(p):
    ...
    expr : expr LE expr
    """
    p[0] = {'node': 'binop', 'op': 'LE', 'left': p[1], 'right': p[3]}
...
def p_expr_gt(p):
    ...
    expr : expr GT expr
    """
    p[0] = {'node': 'binop', 'op': 'GT', 'left': p[1], 'right': p[3]}
...
def p_expr_ge(p):
    ...
    expr : expr GE expr
    """
    p[0] = {'node': 'binop', 'op': 'GE', 'left': p[1], 'right': p[3]}
...
def p_expr_plus(p):
    ...
    expr : expr PLUS expr
    """
    p[0] = {'node': 'binop', 'op': 'PLUS', 'left': p[1], 'right': p[3]}
...
def p_expr_minus(p):
    ...
    """
    p[0] = {'node': 'binop', 'op': 'MINUS', 'left': p[1], 'right': p[3]}
...
def p_expr_times(p):
    ...
    """
    p[0] = {'node': 'binop', 'op': 'TIMES', 'left': p[1], 'right': p[3]}
...
def p_expr_divide(p):
    ...
    """
    p[0] = {'node': 'binop', 'op': 'DIVIDE', 'left': p[1], 'right': p[3]}
...
def p_expr_uminus(p):
    ...
    """
    p[0] = {'node': 'unop', 'op': 'UMINUS', 'value': p[2]}
...
def p_expr_call(p):
    ...
    """
    p[0] = {'node': 'call_expr', 'name': p[1], 'args': p[3]}
